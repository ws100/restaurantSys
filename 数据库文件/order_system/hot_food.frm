TYPE=VIEW
query=select `order_system`.`food`.`Food_id` AS `Food_id`,`order_system`.`food`.`Food_name` AS `Food_name`,`order_system`.`food`.`Food_typeid` AS `Food_typeid`,`order_system`.`food`.`Food_price` AS `Food_price`,`order_system`.`food`.`Food_discription` AS `Food_discription`,`order_system`.`food`.`Food_img_url` AS `Food_img_url`,`order_system`.`food`.`Food_sale` AS `Food_sale` from `order_system`.`food` where (`order_system`.`food`.`Food_sale` = 1)
md5=f05c8dae96221642c96d62e969a35202
updatable=1
algorithm=0
definer_user=root
...
create-version=1
source=SELECT * FROM food WHERE Food_sale = 1
